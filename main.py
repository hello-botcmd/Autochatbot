...
def get_dashboard_text():
    return (
        ...
        "• **💎 Paid Photo** — Set a channel post to forward on `send` / `.send` / `star`\n"
...
async def callback_handler(client, query: CallbackQuery):
    ...
    if query.data == "back_start":
        ...
        text = (
            ...
            "After that, when anyone sends `send`, `.send` or `.star` in that "
            "account's DMs, the saved post is forwarded instantly. AI won't "
            "reply to those trigger words.\n\n"
            "🟢 Online · ⚪ Offline · 📎 Post saved · ➕ No post"
        )
        btns = []
        ...
        btns = [
            [InlineKeyboardButton("📎 Set / Change Post", callback_data=f"photo_set_{target_uid}")],
            [InlineKeyboardButton("🧹 Clear Post", callback_data=f"photo_clear_{target_uid}")],
        ...
        await query.message.edit_text(
            ...
            f"**Step 2:** Copy the link of the post to forward.\n"
        ...
        await query.answer(
            "Saved post cleared!" if cleared else "No saved post on this account.",
            show_alert=True,
        )
    ...
    if state == "AWAITING_SESSION":
        ...
        try:
            ...
            await status_msg.edit_text(
                ...
                f"• **User ID:** `{acc_uid}`\n"
                f"• **Auto Photo Trigger:** Active (`send`, `.send`, `star`)",
                reply_markup=markup,
            )
